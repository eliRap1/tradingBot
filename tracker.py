...
class TradeTracker:
    ...
    def log_stats(self):
        ...
        if stats.get("r_expectancy") is not None:
            log.info(f"  R-Expectancy: {stats['r_expectancy']:+.2f}R | "
                     f"Rolling Sharpe(20): {stats['rolling_sharpe_20']}")
        log.info(f"  Max Drawdown: ${stats['max_drawdown']:,.2f} | "
                 f"Calmar: {stats['calmar_ratio']} | "
                 f"Consec W/L: {stats['max_consecutive_wins']}/{stats['max_consecutive_losses']}")
